...
def benchmark_simulated(results_fname, signature_names, W):
    ...
    with open(results_fname, "w") as report:
        report.write(
            "TEST\tMUTATIONS\tMETH\tROUND\tSIGNATURE\tVALUE\tSE_E\tMSE_M\tMSE_E\tLL\tFROB\tFROB0\tJS\tKL\n"
        )
        # for j in range(N):
            ...
            for N_mutations in [10, 50, 100, 500, 1000, 10000]:
                ...
                for iteration in range(10):
                    ...
                    v0_counts = np.random.multinomial(
                        N_mutations, v0 / v0.sum()
                    )  # np.ceil(v0 * N_mutations)
                    ...
                    for j in range(N):
                        report.write(
                            "{}_{}_{}\t{}\t{}\t{}\t".format(
                                signature_ids[i], 80, 20, N_mutations, "MLE", iteration + 1
                            )
                        )
                        report.write(f"{signature_names[j]}\t{int(h1_counts[j])}\t")
                        SE_E = (h0[j] - h1[j]) ** 2
                        report.write(
                            f"{SE_E}\t{MSE_M}\t{MSE_E}\t{ll}\t{frob}\t{frob0}\t{js}\t{kl}\n"
                        )
