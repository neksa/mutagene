...
class RankMenu:
    def __init__(self, parser):
        ...
        required_group.add_argument(
            "--infile", "-i", help="Input file in MAF format", type=argparse.FileType("r")
        )
        required_group.add_argument(
            "--genome",
            "-g",
        ...
        optional_group.add_argument(
            ...
            default="MAF",
        )  # "Input format: MAF, VCF, or TCGI"
        optional_group.add_argument(
            "--cohorts-file",
            type=str,
            help="Location of tar.gz container or directory for cohorts",
            default="cohorts.tar.gz",
            nargs="?",
        )
        optional_group.add_argument(
            "--cohort",
            "-c",
            type=str,
            help="Name of cohort with observed mutations",
            nargs="?",
            const="",
        )
        ...
        advanced_group.add_argument(
            "--profile",
            "-p",
            help="Override profile to calculate mutability, may also describe cohort size",
            type=str,
        )
        ...
        advanced_group.add_argument(
            "--threshold-driver",
            "-td",
            help="BScore threshold between Driver and Pontential Driver mutations",
            type=float,
            default=THRESHOLD_DRIVER,
        )
        advanced_group.add_argument(
            "--threshold-passenger",
            "-tp",
            help="BScore threshold between Pontential Driver and Passenger mutations",
            type=float,
            default=THRESHOLD_PASSENGER,
        )
    ...
    def callback(self, args):
        ...
        if args.cohort is not None:
            profile, cohort_size, cohort_aa_mutations, cohort_na_mutations = (
                read_cohort_mutations_from_tar(args.cohorts_file, args.cohort)
            )
