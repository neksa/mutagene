...
def export_cohorts(PATH):
    ...
    q = (
        db.query(
            ...
            Signature.name,
            Signature.mut_type,
            Signature.n_mutations,
            Signature.nr_mutations,
            Signature.n_samples,
            Signature.signature_type,
            Signature.cancer_type_id,
            CancerType.name.label("cancer_type"),
            PrimarySite.name.label("primary_site"),
    ...
    q = (
        db.query(
            ...
            Signature.n_samples,
            Signature.signature_type,
            bindparam("cancer_type", "Pan-cancer"),
            bindparam("primary_site", "All tissues"),
            bindparam("cancer_code", ""),
            bindparam("cancer_projects", ""),
        )
    ...
    q = (
        db.query(
            ...
            Signature.n_samples,
            Signature.signature_type,
            bindparam("cancer_type", "SNP"),
            bindparam("primary_site", "Germline"),
            bindparam("cancer_code", ""),
            bindparam("cancer_projects", ""),
        )
    ...
    for profile in profiles:
        ...
        if profile.signature_type == "CT":
            mm = (
                db.query(Mutation)
                .join(SignatureMutation)
                .filter(SignatureMutation.signature_id == int(profile.id))
            )
        elif profile.signature_type == "PC":
            mm = (
                db.query(Mutation)
                .join(SignatureMutation)
                .filter(SignatureMutation.signature_id.in_(all_cancer_types))
            )
        ...
        if len(aa_mutations):
            with open(
                "{}/{}.aa_mutations.txt".format(PATH, profile.name.replace(" ", "_")), "w"
            ) as aa:
        ...
        if len(na_mutations):
            with open(
                "{}/{}.dna_mutations.txt".format(PATH, profile.name.replace(" ", "_")), "w"
            ) as na:
