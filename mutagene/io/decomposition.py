...
def _get_stats(results):
    ...
    df = pd.DataFrame(dict(zip("signature\texposure\tmutations".split(), [signatures, h, m])))
...
def write_decomposition(
    ...
    """
    bootstrap = (
        bootstrap_results is not None and bootstrap_level is not None and profile is not None
    )
    ...
    for sample in samples_results.keys():
        if bootstrap:
            if bootstrap_method == "t":
                ...
                n = int(np.sum(p))
                assert n > 1
                df_sample = _get_bootstrap_stats_t(
                    samples_results[sample], bootstrap_results[sample], n, bootstrap_level
                )
            elif bootstrap_method == "p":
                # percentile-based confidence intervals
                df_sample = _get_bootstrap_stats_percentile(
                    bootstrap_results[sample], bootstrap_level
                )
            else:
