...
def make_NCI60_report(prefix, n=30, decomposition="L"):
    ...
    stats_columns = (
        ...
        "Skipped_indels_C",
        "Skipped_indels_NC",
        "Skipped_nucleotide_C",
        "Skipped_nucleotide_NC",
        "Skipped_context_C",
        "Skipped_context_NC",
    )
    with open(COSMIC_CLINICAL) as f, open(prefix + f"/report_{decomposition}{n}.txt", "w") as o:
        o.write(
            ...
            + "\t".join(stats_columns)
            + "\t"
            + "\t".join([f"{s}{x}" for x in range(1, n + 1)])
            + "\tOther\tUnexplained\n"
        )
...
def read_CLP_extended(f, asm=None, ids=None):
    ...
    for sample, raw_mutations in samples.items():
        ...
        N_loaded = int(sum(mutations.values()))
        processing_stats[sample] = {
            "processed": N,
            "loaded": N_loaded,
            "skipped": N - N_loaded,
            "skipped_indels": samples_skipped_indels[sample],
            "skipped_context": samples_skipped_context[sample],
...
"""
    ...
    for k, v in w["weights"][0].items():
        ...
        if float(v) == 0.0:
            continue
        result.append({"name": k.replace("Signature.", "COSMIC "), "score": v})
    return None, result
    ...
    with open(maf_file, "w") as o:
        ...
        for sample in samples_raw.keys():
            for mutation in chain(samples_raw[sample], samples_raw_NCV[sample]):
                ...
                o.write(
                    f"\t\t\tGRCh38\t{chrom}\t{pos}\t{pos}\t+\t\tSNP\t{x}\t{y}\t{y}\t\t\t{sample}\t{sample}\n"
                )
...
def analyze_nci60_samples(prefix):
    ...
    for sample, mutations in samples.items():
        ...
        with open(oname, "w") as o:
            ...
            o.write(
                "Skipped_nucleotide_NC\t{}\n".format(
                    processing_stats_NCV[sample]["skipped_nucleotide"]
                )
            )
        ...
        for method in "LJRZ":
            ...
            if method == "L":
                _, _, contributing_signatures_A = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLA", "MLE", others_threshold=THRESHOLD
                )
                _, _, contributing_signatures_B = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLB", "MLE", others_threshold=THRESHOLD
                )
                _, _, contributing_signatures_cosmic = decompose_mutational_profile_counts(
                    mutational_profile_counts, "IS", "MLE", others_threshold=THRESHOLD
                )
            if method == "J":
                _, _, contributing_signatures_A = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLA", "js"
                )
                _, _, contributing_signatures_B = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLB", "js"
                )
                _, _, contributing_signatures_cosmic = decompose_mutational_profile_counts(
                    mutational_profile_counts, "IS", "js"
                )
            if method == "R":
                _, _, contributing_signatures_A = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLA", "frobenius"
                )
                _, _, contributing_signatures_B = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLB", "frobenius"
                )
                _, _, contributing_signatures_cosmic = decompose_mutational_profile_counts(
                    mutational_profile_counts, "IS", "frobenius"
                )
            if method == "Z":
                _, _, contributing_signatures_A = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLA", "frobeniuszero"
                )
                _, _, contributing_signatures_B = decompose_mutational_profile_counts(
                    mutational_profile_counts, "CLB", "frobeniuszero"
                )
                _, _, contributing_signatures_cosmic = decompose_mutational_profile_counts(
                    mutational_profile_counts, "IS", "frobeniuszero"
                )

            if method == "D":
                _, _, contributing_signatures_cosmic = deconstruct_sigs(
                    prefix + f"/profiles/{SAMPLE_ID}.profile", SAMPLE_ID
                )
