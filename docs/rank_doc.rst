...
.. mermaid::
   ...
   flowchart TD
       START["ranking inputs"] --> C{"--cohort given?"}
       C -- yes --> FC["profile, cohort size and<br/>observed counts from the cohort"]
       C -- no --> FI["all three from<br/>the input file"]
       FC --> P{"--profile given?"}
       FI --> P
       P -- yes --> FP["profile from the file;<br/>cohort size too, if recorded"]
       P -- no --> N
       FP --> N{"--nsamples given?"}
       N -- yes --> FN["cohort size from --nsamples"]
       N -- no --> END["ranking runs"]
       FN --> END
